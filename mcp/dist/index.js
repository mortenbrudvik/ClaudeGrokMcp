...
import { grokGenerateImageToolDefinition, handleGrokGenerateImage, } from './tools/generate-image.js';
// Services
 ...
 */
export function logServiceStatus(services) {
    ...
    console.error(`[${SERVER_NAME}]   - Cache: ${cacheOpts.enabled ? `enabled (TTL: ${cacheOpts.ttlSeconds}s)` : 'disabled'}`);
    console.error(`[${SERVER_NAME}]   - Cost tracking: limit $${costOpts.limitUsd} (${costOpts.enforceLimit ? 'enforced' : 'warn only'})`);
    console.error(`[${SERVER_NAME}]   - Rate limiting: ${rateLimits.tokensPerMinute.toLocaleString()} TPM`);
}
/**
