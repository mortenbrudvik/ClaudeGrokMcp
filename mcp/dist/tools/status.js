 ...
 */
export function executeGetStatus(services, input) {
...
function formatStatusOutput(response) {
    ...
    lines.push(`## Grok MCP Status: ${statusEmoji[response.status]} ${response.status.toUpperCase()}`);
    lines.push('');
    // Rate Limits
    lines.push('### Rate Limits');
    lines.push(`- **Tokens Remaining:** ${response.rate_limits.tokens_remaining.toLocaleString()}`);
    lines.push(`- **Requests Remaining:** ${response.rate_limits.requests_remaining.toLocaleString()}`);
    ...
    if (response.details) {
        ...
        if (response.details.retry_state.count > 0) {
            lines.push('');
            lines.push(`**Retry State:** ${response.details.retry_state.count} retries, ${response.details.retry_state.delay_ms}ms delay`);
        }
    }
    return lines.join('\n');
}
 ...
 */
export async function handleGrokStatus(services, input) {
    try {
        // Validate services
        if (!services) {
