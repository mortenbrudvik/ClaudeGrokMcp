...
import { validateGrokGenerateImageInput, grokGenerateImageSchema, grokGenerateImageToolDefinition, } from './generate-image.js';
describe('grok_generate_image tool', () => {
    ...
    describe('validateGrokGenerateImageInput', () => {
        ...
        it('should throw on null prompt', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: null })).toThrow('prompt parameter is required');
        });
        it('should throw on empty prompt', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: '   ' })).toThrow('prompt parameter cannot be empty');
        });
        it('should throw on non-string prompt', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 123 })).toThrow('prompt parameter must be a string');
        });
        ...
        it('should throw on n out of range (low)', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', n: 0 })).toThrow('n parameter must be between 1 and 10');
        });
        it('should throw on n out of range (high)', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', n: 11 })).toThrow('n parameter must be between 1 and 10');
        });
        it('should throw on non-integer n', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', n: 2.5 })).toThrow('n parameter must be an integer');
        });
        it('should throw on non-number n', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', n: 'three' })).toThrow('n parameter must be an integer');
        });
        it('should throw on invalid response_format', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', response_format: 'invalid' })).toThrow('response_format must be "url" or "b64_json"');
        });
        it('should throw on non-string response_format', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', response_format: 123 })).toThrow('response_format parameter must be a string');
        });
        it('should throw on non-string model', () => {
            expect(() => validateGrokGenerateImageInput({ prompt: 'test', model: 123 })).toThrow('model parameter must be a string');
        });
        ...
        it('should throw on prompt exceeding maximum length', () => {
            ...
            expect(() => validateGrokGenerateImageInput({ prompt: tooLongPrompt })).toThrow('prompt parameter exceeds maximum length');
        });
...
describe('handleGrokGenerateImage', () => {
    ...
    it('should generate image with default settings', async () => {
        ...
        const result = await handleGrokGenerateImage(mockClient, { prompt: 'A cat' });
    ...
    it('should not include URL expiration notice for b64_json format', async () => {
        ...
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        const result = await handleGrokGenerateImage(mockClient, {
            prompt: 'A cat',
            response_format: 'b64_json',
        });
    ...
    it('should handle multiple images', async () => {
        ...
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        const result = await handleGrokGenerateImage(mockClient, { prompt: 'A cat', n: 2 });
    ...
    it('should use services when provided', async () => {
        ...
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        await handleGrokGenerateImage(mockClient, { prompt: 'A cat' }, mockServices);
    ...
    it('should call generateImage with correct parameters', async () => {
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        await handleGrokGenerateImage(mockClient, {
            prompt: 'A cat',
            n: 3,
            response_format: 'b64_json',
            model: 'grok-2-image',
    ...
    it('should resolve model aliases', async () => {
        ...
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        await handleGrokGenerateImage(mockClient, { prompt: 'A cat', model: 'image' });
    ...
    it('should calculate cost based on number of images', async () => {
        ...
        const { handleGrokGenerateImage } = await import('./generate-image.js');
        await handleGrokGenerateImage(mockClient, { prompt: 'A cat', n: 3 });
...
//# sourceMappingURL=generate-image.test.js.map