 ...
 * @module tools/query
 */
import { XAIError, VISION_CAPABLE_MODELS, WEIGHT_TIERS, } from '../types/index.js';
 ...
 */
export const LONG_OUTPUT_INDICATORS = [
 ...
 */
export function shouldAutoStream(query, context, explicitStream, resolvedModel, isJsonMode, complexityScore) {
    ...
    const isReasoningModel = (resolvedModel.includes('reasoning') && !resolvedModel.includes('non-reasoning')) ||
...
export function validateGrokQueryInput(input) {
    ...
    return {
        ...
        top_p: params.top_p,
        stream: params.stream, // Preserve undefined to allow auto-streaming
...
export async function handleGrokQuery(client, args, services) {
    try {
        ...
        const streamingDecision = shouldAutoStream(input.query, input.context, input.stream, resolvedModel, isJsonMode, autoSelection?.complexityScore);
