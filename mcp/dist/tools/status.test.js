...
import { executeGetStatus, handleGrokStatus, grokStatusSchema, grokStatusToolDefinition, } from './status.js';
...
describe('grok_status tool', () => {
    ...
    describe('executeGetStatus', () => {
        ...
        it('should include details when include_details is true', () => {
            const services = createMockServices({
                ...
                rateLimiterOptions: { tier: 'enterprise' },
    ...
    describe('handleGrokStatus', () => {
        ...
        it('should return error when services not provided', async () => {
            const result = await handleGrokStatus(null, {});
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('Services not available');
        });
