 ...
 * @module tools/models
 */
import { MODEL_ALIASES, MODEL_PRICING, } from '../types/index.js';
/**
 ...
 */
export function validateGrokModelsInput(input) {
 ...
 * ```
 */
export async function executeGrokModels(client, input) {
...
function formatModelRow(model) {
    ...
    const pricing = model.pricing.input_per_1m > 0
        ? `$${model.pricing.input_per_1m}/$${model.pricing.output_per_1m}`
        : 'Unknown';
    const context = model.context_window >= 1000000
 ...
 */
export async function handleGrokModels(client, args) {
    try {
        // Validate input
