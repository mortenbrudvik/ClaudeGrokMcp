...
function buildFilePrompt(query, fileContent, fileType, filename, context) {
    ...
    parts.push('');
    parts.push('Please analyze the file and answer the query. Be specific and reference relevant parts of the file when applicable.');
    return parts.join('\n');
}
/**
 * Validate input parameters
 */
export function validateGrokWithFileInput(input) {
    ...
    if (params.file_type !== undefined) {
        if (typeof params.file_type !== 'string' ||
            !validFileTypes.includes(params.file_type)) {
 ...
 * Execute file query
 */
export async function executeGrokWithFile(client, input) {
    ...
    if (stats.size_bytes > MAX_RECOMMENDED_SIZE) {
        console.error(`[grok_with_file] Warning: File size (${stats.size_bytes} bytes) exceeds recommended limit (${MAX_RECOMMENDED_SIZE} bytes). Consider truncating for better performance.`);
    }
    // Build the prompt
    const prompt = buildFilePrompt(input.query, input.file_content, fileType, input.filename, input.context);
...
function formatWithFileOutput(result) {
    ...
    lines.push(`**File:** ${fileInfo.filename || 'unnamed'} | **Type:** ${fileInfo.detected_type} | **Lines:** ${fileInfo.line_count}`);
    lines.push('');
    // Response
    lines.push('### Response');
    lines.push('');
 ...
 */
export async function handleGrokWithFile(client, input, services) {
    try {
        // Validate input
        ...
        const estimatedInputTokens = Math.ceil((validatedInput.file_content.length +
...
//# sourceMappingURL=with-file.js.map