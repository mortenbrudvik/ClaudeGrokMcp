...
import { TokenUsage, CostEstimate, ChatMessage, Services } from '../types/index.js';
...
export const reasonSchema = {
  ...
  properties: {
    ...
    effort: {
      ...
      default: 'medium',
      description:
        'Reasoning effort level: low (quick analysis), medium (balanced), high (thorough deep thinking)',
    },
    show_thinking: {
      type: 'boolean',
      default: true,
 ...
 */
function parseReasoningResponse(response: string): { thinking: string; answer: string } {
 ...
 */
function formatReasonOutput(result: ReasonResponse): string {
  const lines: string[] = [];
...
export async function handleReason(
  ...
  try {
    ...
    const reasonInput: ReasonInput = {
      query: params.query,
      effort: typeof params.effort === 'string' ? (params.effort as ReasoningEffort) : undefined,
