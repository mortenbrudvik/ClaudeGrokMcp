...
import {
  validateGrokExecuteCodeInput,
  grokExecuteCodeSchema,
...
describe('grok_execute_code tool', () => {
  ...
  describe('validateGrokExecuteCodeInput', () => {
    ...
    it('should throw on empty code', () => {
      expect(() => validateGrokExecuteCodeInput({ code: '' })).toThrow(
        'code parameter cannot be empty'
      );
    });

    it('should throw on non-string code', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 123 })).toThrow(
    ...
    it('should throw on invalid description type', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', description: 123 })).toThrow(
        'description parameter must be a string'
      );
    });

    it('should throw on invalid include_output type', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', include_output: 'yes' })).toThrow(
        'include_output parameter must be a boolean'
      );
    });

    it('should throw on invalid max_turns type', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', max_turns: 'five' })).toThrow(
        'max_turns parameter must be an integer'
      );
    });

    it('should throw on max_turns out of range (low)', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', max_turns: 0 })).toThrow(
        'max_turns parameter must be between 1 and 10'
      );
    });

    it('should throw on max_turns out of range (high)', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', max_turns: 11 })).toThrow(
        'max_turns parameter must be between 1 and 10'
      );
    });

    it('should throw on invalid model type', () => {
      expect(() => validateGrokExecuteCodeInput({ code: 'x', model: 123 })).toThrow(
    ...
    it('should throw on code exceeding maximum length', () => {
      ...
      expect(() => validateGrokExecuteCodeInput({ code: tooLongCode })).toThrow(
        'code parameter exceeds maximum length'
      );
    });
  });
});
...
describe('handleGrokExecuteCode', () => {
  ...
  it('should execute code with default settings', async () => {
    ...
    const result = await handleGrokExecuteCode(mockClient as any, { code: 'print("hello")' });
  ...
  it('should include code_interpreter tool in request', async () => {
    const { handleGrokExecuteCode } = await import('./execute-code.js');
    await handleGrokExecuteCode(mockClient as any, { code: 'print("hello")' });
  ...
  it('should include description in user message when provided', async () => {
    const { handleGrokExecuteCode } = await import('./execute-code.js');
    await handleGrokExecuteCode(mockClient as any, {
      code: 'print("hello")',
      description: 'Print a greeting',
    });

    const call = mockClient.responsesCreate.mock.calls[0][0];
    const userMessage = call.input.find((m: any) => m.role === 'user');
  ...
  it('should detect error responses', async () => {
    ...
    const { handleGrokExecuteCode } = await import('./execute-code.js');
    const result = await handleGrokExecuteCode(mockClient as any, { code: '1/0' });
  ...
  it('should use services when provided', async () => {
    ...
    const { handleGrokExecuteCode } = await import('./execute-code.js');
    await handleGrokExecuteCode(mockClient as any, { code: 'print("hello")' }, mockServices as any);
  ...
  it('should respect max_turns parameter', async () => {
    const { handleGrokExecuteCode } = await import('./execute-code.js');
    await handleGrokExecuteCode(mockClient as any, { code: 'print("hello")', max_turns: 5 });
