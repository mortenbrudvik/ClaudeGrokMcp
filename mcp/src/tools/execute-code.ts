...
import {
  GrokExecuteCodeInput,
  GrokExecuteCodeResponse,
  AgentToolsParams,
  CodeExecutionConfig,
  XAIError,
  Services,
 ...
 */
export function validateGrokExecuteCodeInput(input: unknown): GrokExecuteCodeInput {
  ...
  return {
    ...
    include_output: (params.include_output as boolean) ?? true,
    max_turns: (params.max_turns as number) ?? DEFAULT_MAX_TURNS,
 ...
 */
function buildUserMessage(code: string, description?: string): string {
  let message = '';
 ...
 */
function formatResponse(result: GrokExecuteCodeResponse): string {
 ...
 */
export async function handleGrokExecuteCode(
  client: XAIClient,
  args: unknown,
  services?: Services
  ...
  try {
    ...
    const codeExecutionTool: CodeExecutionConfig = {
    ...
    // Build result object
    const result: GrokExecuteCodeResponse = {
