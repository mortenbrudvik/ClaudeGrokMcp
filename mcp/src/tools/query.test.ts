...
describe('grok_query tool', () => {
  ...
  describe('validateGrokQueryInput', () => {
    ...
    describe('invalid inputs', () => {
      ...
      it('should reject non-integer timeout', () => {
        expect(() => validateGrokQueryInput({ query: 'test', timeout: '30000' })).toThrow(
          'timeout must be an integer'
        );
        expect(() => validateGrokQueryInput({ query: 'test', timeout: 30000.5 })).toThrow(
          'timeout must be an integer'
        );
      });

      it('should reject timeout out of range', () => {
        expect(() => validateGrokQueryInput({ query: 'test', timeout: 999 })).toThrow(
          'timeout must be between 1,000 and 120,000 milliseconds'
        );
        expect(() => validateGrokQueryInput({ query: 'test', timeout: 120001 })).toThrow(
          'timeout must be between 1,000 and 120,000 milliseconds'
        );
      });
    ...
    describe('vision support (P4-015)', () => {
      ...
      it('should reject non-string image_url', () => {
        expect(() =>
          validateGrokQueryInput({
            query: 'test',
            image_url: 123 as unknown as string,
          })
    ...
    describe('top_p parameter', () => {
      ...
      it('should reject non-number top_p', () => {
        expect(() => validateGrokQueryInput({ query: 'test', top_p: '0.5' })).toThrow(
          'top_p must be a number'
        );
      });

      it('should reject top_p out of range', () => {
        expect(() => validateGrokQueryInput({ query: 'test', top_p: -0.1 })).toThrow(
          'top_p must be between 0 and 1'
        );
        expect(() => validateGrokQueryInput({ query: 'test', top_p: 1.1 })).toThrow(
          'top_p must be between 0 and 1'
        );
      });
    });
  ...
  describe('handleGrokQuery with services', () => {
    ...
    it('should return cached response on cache hit', async () => {
      ...
      const cacheKey = mockServices.cache.generateKey(
        'Hello',
    ...
    it('should check budget before API call', async () => {
      ...
      const result = await handleGrokQuery(
        client,
        { query: 'This should be rejected' },
        servicesWithLowBudget
      );
  ...
  describe('UX enhancements', () => {
    ...
    it('should show complexity score for code queries (P4-011)', async () => {
      ...
      const result = await handleGrokQuery(
        client,
        { query: 'Fix this bug in my function', model: 'auto' },
        mockServices
      );
    ...
    it('should show cost savings tip for expensive models', async () => {
      ...
      const result = await handleGrokQuery(
        client,
        { query: 'Complex query', model: 'smartest', max_tokens: 2000 },
        mockServices
      );
  ...
  describe('streaming support', () => {
    ...
    describe('handleGrokQuery with streaming', () => {
      it('should route streaming requests to streaming executor', async () => {
        ...
        const result = await handleGrokQuery(
          client,
          { query: 'Streaming query', stream: true },
          mockServices
        );
      ...
      it('should skip cache for streaming requests', async () => {
        ...
        const result = await handleGrokQuery(
          client,
          { query: 'Streaming bypass', stream: true },
          mockServices
        );
      ...
      it('should handle streaming API errors gracefully', async () => {
        ...
        const result = await handleGrokQuery(
          client,
          { query: 'Streaming error test', stream: true },
          mockServices
        );
      ...
      it('should resolve model aliases correctly in streaming mode', async () => {
        ...
        await handleGrokQuery(
          client,
          { query: 'Test alias resolution', model: 'fast', stream: true },
          mockServices
        );
  ...
  describe('JSON mode (P4-016)', () => {
    ...
    describe('validateGrokQueryInput', () => {
      ...
      it('should reject invalid response_format type', () => {
        expect(() =>
          validateGrokQueryInput({
            query: 'test',
            response_format: { type: 'text' as 'json_object' },
          })
      ...
      it('should reject non-object response_format', () => {
        expect(() =>
          validateGrokQueryInput({
            query: 'test',
            response_format: 'json_object' as unknown as { type: 'json_object' },
          })
      ...
      it('should reject null response_format', () => {
        expect(() =>
          validateGrokQueryInput({
            query: 'test',
            response_format: null as unknown as { type: 'json_object' },
          })
        ).toThrow('response_format must be an object');
      });
    ...
    describe('handleGrokQuery with JSON mode', () => {
      it('should show JSON valid indicator in formatted output', async () => {
        ...
        const result = await handleGrokQuery(
          client,
          { query: 'Return JSON', response_format: { type: 'json_object' } },
          mockServices
        );
    ...
    describe('cache key differentiation', () => {
      it('should use different cache keys for JSON vs non-JSON mode', async () => {
        ...
        const jsonResult = await handleGrokQuery(
          client,
          { query: 'Same query', model: 'fast', response_format: { type: 'json_object' } },
          mockServices
        );
