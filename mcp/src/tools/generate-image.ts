...
export const grokGenerateImageSchema = {
  ...
  properties: {
    ...
    response_format: {
      ...
      default: 'url',
      description:
        'Response format: url (temporary URLs, easier to view) or b64_json (base64 data, permanent)',
    },
    model: {
      type: 'string',
...
export const grokGenerateImageToolDefinition = {
  ...
  description:
    "Generate images from text descriptions using xAI's Grok image models. " +
 ...
 */
export function validateGrokGenerateImageInput(input: unknown): GrokGenerateImageInput {
  ...
  return {
    ...
    n: (params.n as number) ?? DEFAULT_N,
    response_format: (params.response_format as 'url' | 'b64_json') ?? 'url',
 ...
 */
function formatImageEntry(image: GeneratedImage, index: number): string {
  let entry = '';
 ...
 */
function formatResponse(result: GrokGenerateImageResponse): string {
  const lines: string[] = [];
 ...
 */
function formatErrorResponse(errorMessage: string, responseTime: number): string {
 ...
 */
export async function handleGrokGenerateImage(
  client: XAIClient,
  args: unknown,
  services?: Services
  ...
  try {
    ...
    // Build result
    const result: GrokGenerateImageResponse = {
      images: response.data,
      count: response.data.length,
      model,
      cost,
      response_time_ms: responseTime,
      url_expiration_notice:
