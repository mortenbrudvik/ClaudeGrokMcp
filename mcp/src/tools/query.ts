 ...
 */
const MODEL_COST_TIERS: Record<
  string,
  { tier: 'cheap' | 'moderate' | 'expensive'; outputRate: number }
> = {
 ...
 */
function getCheaperAlternatives(currentModel: string): Array<{ model: string; savings: string }> {
 ...
 */
function parseJsonResponse(response: string): JsonParseResult {
...
export const grokQuerySchema = {
  ...
  properties: {
    ...
    top_p: {
      type: 'number',
      description:
        'Nucleus sampling: only consider tokens with top_p cumulative probability (0-1). Alternative to temperature. Recommend altering top_p OR temperature, not both.',
      minimum: 0,
      maximum: 1,
    },
    stream: {
    ...
    image_detail: {
      ...
      enum: ['auto', 'low', 'high'],
      description:
        'Detail level for image analysis. "low" uses fewer tokens, "high" provides more detail. Default: "auto".',
      default: 'auto',
    },
    response_format: {
      type: 'object',
      description:
        'Request structured JSON output. When set, Grok will return valid JSON. Use { type: "json_object" }.',
      properties: {
        type: {
          type: 'string',
          enum: ['json_object'],
...
export function validateGrokQueryInput(input: unknown): GrokQueryInput {
  ...
  if (params.image_detail !== undefined) {
    if (!['auto', 'low', 'high'].includes(params.image_detail as string)) {
 ...
 */
export async function executeGrokQueryStreaming(
  client: XAIClient,
  input: GrokQueryInput
): Promise<StreamingGrokQueryResponse> {
  const startTime = Date.now();
  const isJsonMode = input.response_format?.type === 'json_object';
...
/**
 * UX enhancement options for response formatting
 */
interface FormatOptions {
  /** Auto-selection info when 'auto' model was used */
  autoSelection?: AutoModelSelection;
  /** Cache hit info including TTL remaining */
  cacheInfo?: {
    isCached: boolean;
    expiresIn?: number; // seconds remaining
  };
  /** Budget status for warnings */
  budgetInfo?: {
    usedPercent: number;
    remainingUsd: number;
    limitUsd: number;
  };
  /** Whether this was an expensive query */
  expensiveQueryWarning?: boolean;
  /** User's original model input (to detect 'auto') */
  originalModelInput?: string;
...
function formatResponse(
  ...
  if (
    options.originalModelInput === 'auto' &&
  ...
  if (options.budgetInfo) {
    if (options.budgetInfo.usedPercent >= UX_THRESHOLDS.BUDGET_WARNING_LEVEL_2) {
      ...
      warnings.push(
        `💰 Budget: ${options.budgetInfo.usedPercent.toFixed(0)}% used ($${options.budgetInfo.remainingUsd.toFixed(2)} remaining)`
      );
    }
  }
...
export async function handleGrokQuery(
  ...
  try {
    ...
    let autoSelection: AutoModelSelection | undefined;
    ...
    try {
      ...
      let result: GrokQueryResponse | StreamingGrokQueryResponse;
      let streamingInfo: FormatOptions['streamingInfo'];
      ...
      let budgetInfo: FormatOptions['budgetInfo'];
      if (services?.costTracker) {
