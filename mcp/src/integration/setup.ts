...
beforeAll(async () => {
  ...
  if (!apiKey) {
    throw new Error(
      'XAI_API_KEY environment variable is required for live tests.\n' +
...
afterAll(() => {
  ...
  if (totalCost > MAX_TEST_BUDGET_USD) {
    throw new Error(
      `Test run exceeded budget: $${totalCost.toFixed(6)} > $${MAX_TEST_BUDGET_USD}\n` +
 ...
 */
export function skipIfApiUnavailable(): boolean {
  if (!isApiAvailable()) {
