...
describe('Cost Tracking (live)', () => {
  ...
  describe('cumulative tracking', () => {
    it('should track cumulative cost across queries', async () => {
      ...
      const cost1 = extractCostFromResponse((r1.content[0] as { text: string }).text);
      expect(cost1).not.toBeNull();
      tracker.addCost('grok-4-fast', 10, 5); // Approximate tokens

      // Second query
      const r2 = await withRateLimit(() =>
        handleGrokQuery(client, {
          query: 'Say: two',
          model: 'fast',
          max_tokens: 5,
      ...
      const cost2 = extractCostFromResponse((r2.content[0] as { text: string }).text);
  ...
  describe('usage summary', () => {
    it('should provide detailed usage summary', async () => {
      ...
      const cost = extractCostFromResponse((result.content[0] as { text: string }).text);
