...
### Specify Model
```
/query --model fast Explain what a mutex is
```

### Code Analysis
```
/query --model code Review this function for bugs: function add(a, b) { return a - b; }
```

### Extended Reasoning
```
/query --model reasoning Think through the trade-offs of microservices vs monolith
```

### Lower Temperature for Factual
```
/query --temperature 0.2 What is the time complexity of quicksort?
```
...
When the user runs `/query`, execute the following:
