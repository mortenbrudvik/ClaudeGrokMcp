...
```
/debug-with-grok <error_description>
```
...
```
/debug-with-grok TypeError: Cannot read property 'x' of undefined
/debug-with-grok The API returns 500 but only in production
/debug-with-grok Tests pass locally but fail in CI
/debug-with-grok Memory usage keeps growing over time
```
...
- `/review-with-grok` - Code review for bug prevention
